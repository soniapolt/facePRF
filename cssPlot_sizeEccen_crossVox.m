...
for r = 1:length(ROIs)
    ...
    [errorObj,lineObj] = scatterline([fits(c).vox.eccen],[fits(c).vox.size],fitRange(N>9),NaN,1000,roiColors(ROIs{r})*mult,0,0); hold on;
    extX = [0:.1:6];
...
if saveFig == 1
    ...
    figure(f(1));
    niceSave([dirOf(pwd) 'figures/' expt '/sizeEccen/'],['scatter_' txt],[],[],{'png' 'svg'}); % just save pngs, since these can be generated pretty quickly
    figure(f(2));
    niceSave([dirOf(pwd) 'figures/' expt '/sizeEccen/'],['lines_' txt],[],[],{'png' 'svg'}); % just save pngs, since these can be generated pretty quickly
end
 playSound;