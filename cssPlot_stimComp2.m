...
if saveFig == 1
    txt = ['subj' subj '_' compStims{1} 'v' compStims{2} '_' fits(whichCond).cond '_'];
    if length(hems) == 1
        txt = [txt '_' hems{1}]; end
    niceSave([dirOf(pwd) 'figures/' expt '/stimComp/'],txt,ROIs); % just save pngs, since these can be generated pretty quickly
end
