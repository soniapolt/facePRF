...
for r = ROInum
    ...
    for c = 1:length(boot.roi(1).cond)
        ...
        plotCovIm(meanIm,boot.res,boot.ppd,1,plotCent,[]); hold on;
...
for rr = 1:length(ROInum)
    ...
    for c = 1:2
        for s = 1:length(subjNum)
            [boot.roi(r).cond(c).centX(s),boot.roi(r).cond(c).centY(s)] = FWHMcentroid(squeeze(boot.roi(r).cond(c).cov(s,:,:)),.5);
        end
    end
...
for n = 1:length(plotPars)
  ...
  title(plotPars{n});
   xticks([1:length(ROInum)]); xticklabels(ROIs); ylabel(['Centroid of FWHM (dva)']);
...
niceSave([raid 'invPRF/figures/fixPRF/coverage/centroid/'],[hemText(hems) '_heirarchy_scale' num2str(boot.scaleSubjs)],[],[],{'svg'});
...
%         
%         for s = 1:length(subjNum)
%             eval(['toPlot(rr,s) =  boot.roi(r).cond(2).' plotPars{n} '(s)-boot.roi(r).cond(1).' plotPars{n} '(s);']);
%         end
%     end
%     
%     % recenter, rescale
...
%             toPlot = -(toPlot-boot.res/2)/boot.ppd;
%         case 'area'
%             toPlot = toPlot/boot.ppd;
%     end
%     subplot(length(plotPars),1,n);
%     scatterErr([1:length(ROInum)],nanmean(toPlot,2),se(toPlot')',roiColors(ROIs));
%     title(plotPars{n});
%     xticks([1:length(ROInum)]); xticklabels(ROIs); ylabel(['centroid estimate']);
%     
%     xlim([0 length(ROIs)+1]);
%     l = hline(0,'k:');set(l,'LineWidth',1.5);hold on;
% end
% %niceSave([raid 'invPRF/figures/fixPRF/coverage/attempt/'],[hemText(hems) '_heirarchy_scale' num2str(boot.scaleSubjs)],[],[],{'svg'});
