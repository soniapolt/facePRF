...
clear all; close all;
dawnDir = [pwd '/dawnData/'];
prfSet = 'analyzePRF_bi_pRFset_ve20.mat';
load([dawnDir '/analyzePRF_origMats/' prfSet]);

minR2 = 20; % default is 10 in this set - to further trim, edit this
ROIs= {'IOG_faces', 'pFus_faces', 'mFus_faces', 'pSTS_faces'};
% info.subjs = {info.subjs}
...
boot.maxEccen = 50;   % in deg, max eccentricity
boot.doTrim = 0;
    ...
    for s = 1:length(info.subjs)
        for r =  1:length(ROIs)
            ...
            if ~isempty(subj(s).roi(r).vox) % if this subject has this ROI
                ...
                %%% try to find values that the algorithm has gotten stuck on
        ...
        case 'hist'
            for t = 1:length(pos)
                for r = 1:length(ROIs)
                    ...
                    %niceSave(miscPath,['SP-comparison_position-' pos{t} '_' hem '-' ROIs{r} '_' fileName(grpCov)]);
