...
%     axis square; g = legend([lo{fliplr(1:length(ROIs))}],fliplr(ROIs(1:4))); set(g,'box','off','FontSize',fontSize+4,'location','NorthEastOutside','Interpreter','none');
%     xlim([.25 6]); ylim([0.25 6]); set(gca,'TickDir','out');
%     subplot(1,2,2);
%     xlabel('Eccen (dva)','FontSize',fontSize); ylabel(['Size (2*SD/sqrt(N)) (dva)'],'FontSize',fontSize); title('Size by Eccentricity','fontSize',titleSize);%ylim([0 2.5]);
%     axis square; g = legend([lo{fliplr(1:length(ROIs))}],fliplr(ROIs(1:4))); set(g,'box','off','FontSize',fontSize+4,'location','NorthEastOutside','Interpreter','none');
%     xlim([.25 6]); ylim([.25 6]); set(gca,'TickDir','out');
% 
...
if saveFig == 1
    ...
    figure(f(1));
    niceSave([dirOf(pwd) 'figures/' expt '/sizeEccen/'],['scatter_' txt '_' bootMethod '_dotAlpha' num2str(doAlpha)],[],[],{'png' 'svg'}); % just save pngs, since these can be generated pretty quickly
    if length(f>1) figure(f(2));
        niceSave([dirOf(pwd) 'figures/' expt '/sizeEccen/'],['lines_' txt '_' bootMethod],[],[],{'png' 'svg'}); % just save pngs, since these can be generated pretty quickly
    end
end
playSound;