...
fitSuffix = '';%'_orig';%
whichCond = 1;
allStims = {'photo','eyes','internal','external'};
fitsSuffix = ''; %'_orig';
compConds = [2 1];
...
plotType = {'scatter' 'distr' 'box' 'box'}; % 1 = boxplot, 2 = distr, 3 = scatter
hems = {'rh' 'lh'};
fitSuffix = '';
...
%             end
        ...
        switch plotType{p}
            ...
            case 'distr'
                nBins = 20;
                plotDistr(plPars,1,{fits(compConds(1)).cond fits(compConds(2)).cond},nBins,3);
                xlim([-5 5]);
            case 'scatter'
                hold on;
        ...
        title([allStims{t} ' ' plotPars{p}],'fontSize',titleSize,'interpreter','none','FontWeight','bold'); 
        pl = pl+1;
        axis square;
    end
...
if saveFig == 1
    ...
    if length(plotConds)==1
        txt = [fits(plotConds).cond '_' txt]; end
    
    niceSave([dirOf(pwd) 'figures/' expt '/stimComp/'],txt); % just save pngs, since these can be generated pretty quickly
end
