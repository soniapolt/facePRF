...
for c = 1:2
    ...
    for r = 1:length(ROIs)
        ...
        if ~exist([sim.name '_cond' num2str(c) '.mat'])
            error(sprintf(['Missing ' [sim.name '_cond' num2str(c)] '.mat! Run prfRec_optim_monteCarlo.m to compute...\n']));
        else
            load([sim.name '_cond' num2str(c)]);
            if r == 1 % load internal features image for visualization
                load('internalAvg.mat');
                faceSize = sim.faceSize*sim.ppd;
                face = imresize(avgFace,[faceSize faceSize]);
                if c == 1 face = flipud(face); end % inverted condition
            end
        end
        XY = vertcat(sim.subj.bestPos);
