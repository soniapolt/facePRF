...
ROIs= standardROIs;%'face');%{'V1'};%['hV4' standardROIs('face')];% %{'PL' 'ML'};%
...
for r = 1:length(ROIs)
    ...
    % plotCoverage(vox,color,leg,ppd,res,plotSize,r2Gain,sampleVox,centerMass,plotCirc)
    if c == 1 mult = .25; else mult = 1; end
    plotCoverage(fits(c).vox,roiColors(ROIs(r))*mult,'',roi(1).fits(1).ppd,roi(1).fits(1).res,plotSize,1,sampleVox);%,1,plotRect/2);
...
%     hold on; rectangle('Position',[-plotRect/2,-plotRect/2,plotRect,plotRect]); end
    ...
    if saveFig
        ...
        niceSave([dirOf(pwd) 'figures/' expt '/coverage/dots/'],txt,[],subjs,{'svg' 'png'}); % just save pngs, since these can be generated pretty quickly
    end
end % ROIs
