    ...
    for c = 1:2
        ...
        switch whichPlot
            ...
            case 'split'
                ...
                %                 subplot(3,length(ROIs),r+(length(ROIs)*2))
                %                 hold on; errorbar([edges(1:end-1)],mean(counts),se(counts),'Color',roiColors(ROIs{r})*(c*.5));
                %
                %                 colormap('parula');colorbar;caxis([yl(1) yl(2)]); % just for sizing
                %                 xticks([edges(1:end)]); xlabel('bin centers');%xticklabels(edges(1:end-1));
