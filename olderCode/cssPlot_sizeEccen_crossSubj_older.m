...
% 5/6/20 changed size to equal 1 x sigma/sqrt(n), not 2x
clear all; close all;
...
ROIs= [standardROIs(1) standardROIs('face')];%standardROIs(7);%('face');% 'V3' 'hV4' 'IOG_faces' 'pFus_faces','mFus_faces'
saveFig = 1;
sampleVox = 0;
...
for r = 1:length(ROIs)
    ...
    for s = 1:length(subj)
        ...
        x = [fits(c).vox.eccen]';
        y = [fits(c).vox.size]'/2; % 5/6/20 correction so that size = 1 sigma / sqrt(n)
        ...
        figure(f(1));
    xl = xlim; xlim([0.25 6]); yl = ylim; if containsTxt(ROIs{r},'faces') ylim([0 6]); else ylim([0 6]);end
    set(gca,'TickDir','out');
    ...
    for c = 1:length(fits)
        if c == 1 mult = .25; else mult = 1; end
            s(c) = scatter([fits(c).vox(sv).eccen],[fits(c).vox(sv).size]/2,2,roiColors(ROIs{r})*mult,'filled'); hold on;
...
if saveFig == 1
    ...
    niceSave([dirOf(pwd) 'figures/' expt '/sizeEccen/randomE/'],['acrossSubjs_sizeCorr_summary_' txt],[],[],{'png' 'svg'}); % just save pngs, since these can be generated pretty quickly
    end
playSound;